...
function handleRefresh() {
	for (var i=1; i<14000; i=i+1000 )//16번 호출 i=1 j=1000, i=1001 j=2000, i=2001 j=3000,..., i=15001 j=16000 까지
	{
	var j = i + 999;
    var url="http://openapi.seoul.go.kr:8088/7a49714b4c67686536307854657056/json/Mgiscampinginfoseoul/"+i+"/"+j;
    $.getJSON(url, updateCamping);
...
function updateCamping(camping) {//16번 호출
   var camping = camping.Mgiscampinginfoseoul.row;
   var addr = "";
   ...
   for (var i = 0; i < camping.length; i++) {
      ...
      var loc = {//open API의 값들 위도와 경도
            latitude :	cam.COT_COORD_X,
			longitude: cam.COT_COORD_Y
      };
	  var km = computeDistance (position, loc); //거리 계산, position 지도의 중심좌표이고 loc는 각 주차장 좌표
	  if(addr != cam.COT_ADDR_FULL_OLD){//주소가 중복되지 않고, 거리가 2km이내의 것들을 가져온다.
		  addr =cam.COT_ADDR_FULL_OLD;
		  
		  addMarker(imageSrc, imageSize, imageOption, cam.COT_COORD_X,cam.COT_COORD_Y, cam.COT_CONTS_NAME, cam.COT_ADDR_FULL_OLD, cam.COT_TEL_NO, cam.COT_VALUE_01);
   ...
   //밑에 리스트 추가하는 부분
   var campingDiv = document.getElementById("camping");
   campingDiv.innerHTML="";
	
	for(var i=0; i<camping.length; i++){
		var cam = camping[i];
		var div = document.createElement("div");
		div.setAttribute("class", "camping");
		
		div.innerHTML="";
		if (cam.COT_GU_NAME== gus && cam.COT_GU_NAME != "") {
			div.innerHTML = "[" + cam.COT_GU_NAME + "]" + cam.COT_CONTS_NAME;
			
			div.innerHTML += "<input type=button value=위치" + " onclick=\"window.open('http://www.google.co.kr/maps/search/" + cam.COT_CONTS_NAME + " ')\"/>"; 
			
			
			if (cam.COT_ADDR_FULL_OLD != "") {
				div.innerHTML += "<br>" + "주소: " + cam.COT_ADDR_FULL_OLD + "(x:"
						+ cam.COT_COORD_Y+ "&nbsp;,&nbsp;y:" +  cam.COT_COORD_X+ ")";
			}
			
			
		if(campingDiv.childElementCount==0){
			campingDiv.appendChild(div);
		}
		else{
			campingDiv.insertBefore(div, campingDiv.firstChild);
		}
		}
		
	}
	if(camping.length > 0){
		lastReportTime = camping[camping.length-1].time;
	}
}
...
function addMarker(imageSrc, imageSize, imageOption, latitude, longitude, name, address,tel, closeday) {
	...
	markerPosition = new daum.maps.LatLng(longitude, latitude);
	var marker = new daum.maps.Marker({
	...
	var content =  "<div style='width:100%; height:100%; padding:5px; font-size:0.8em;'>"+"캠핑장 : "+ '<b>'+ name +'</b>'+'<br>'+"주소 : " +address+'<br>'+"전화번호 : "+tel+'<br>'+"설명 : " +'<font color="red" >'+closeday+'</font>'+ "</div>";
